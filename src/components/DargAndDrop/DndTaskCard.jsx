...
export const DndTaskCard = ({ id, index,  moveCard,task, label, CaseAllocation,  moveFirstBoxCard}) => {
  ...
  return (
    ...
    >
    <Box className=" flex flex-col gap-y-11 px-6 xl:px-0 py-4 relative mb-10">
                    <Box className="xl:px-6 flex items-center justify-between ">
                      {/* avatar  */}
                      <Box>
                    ...
                    <Box className="xl:pl-6 flex items-center justify-between" id={task}>
                      <Typography sx={{ color: theme.palette.text.black }}>{label}</Typography>
                      <Box className={` w-5 h-5 rounded-full   -mr-3 hidden lg:block `} sx={{ bgcolor: theme.palette.primary.cardLightBg }}>
                        {/* parent root  */}
